# Fuzzer — Type-Aware Function Fuzzing & Equivalence Checking

Two standalone scripts for randomly testing Python functions:

| Script | Purpose |
|---|---|
| `fuzz_function.py` | Fuzz a single function with random inputs and display outputs |
| `equivalence_checker.py` | Check if two functions are equivalent via differential fuzzing |

Both scripts use the **Python AST** to parse type annotations directly from
function signatures, so they support arbitrarily complex nested types such as
...
### 2. Check Equivalence of Two Functions
...
**Non-equivalent example** — in `pair2.py`:
...
| Tuples | `tuple[T1, T2, ...]` (fixed), `tuple[T, ...]` (variadic) |
| Optional | `Optional[T]`, `T \| None` |
| Union | `Union[T1, T2]`, `T1 \| T2` |

These can be **arbitrarily nested**, e.g.:
...
Both scripts can also be imported and used as a library:

```python
from fuzzer.type_parser import extract_function_signature
...
├── fuzz_function.py          # Script 1: Fuzz a single function
├── equivalence_checker.py    # Script 2: Equivalence checker
└── README.md                 # This file
```
